...
BOOL CAddinSettingsDialog::OnInitDialog() 
{
	...
	for(int i=0;i < CProjectFrameworkApp::m_AddinManager.GetAddinCount();i++)
	{
		int index=m_ListAddin.AddString(CProjectFrameworkApp::m_AddinManager.GetAddinInfo(i).m_strAddinName);
		m_ListAddin.SetCheck(index,CProjectFrameworkApp::m_AddinManager.GetAddinInfo(i).m_bLoadAddin); 
	}
...
void CAddinSettingsDialog::OnOK() 
{
	for(int i=0;i<m_ListAddin.GetCount();i++)
	{
		...
		CProjectFrameworkApp::m_AddinManager.SetAddinVisible(strAddinText,m_ListAddin.GetCheck(i));
	}
	CDialog::OnOK();
}
