...
/* ----------------------------------- File Header -------------------------------------------*
	File				:	ProjectFrameworkExt.cpp
	Project Code		:	
	Author	    		:	Tom Thomas
	Created On	    	:	10/28/2004 6:11:11 PM
	Last Modified	   	:	10/28/2004 6:11:11 PM
----------------------------------------------------------------------------------------------*
	Type				:	C++ Source File
	Description			:   
	Developer's Note	:	
...
// ProjectFrameworkExt.cpp : Implementation of CProjectFramework
#include "stdafx.h"
#include "ProjectFramework.h"
#include "ProjectFrameworkExt.h"
...
STDMETHODIMP CProjectFramework::InterfaceSupportsErrorInfo(REFIID riid)
{
...
STDMETHODIMP CProjectFramework::SetAddInInfo(long lSessionID, long lInstanceHandle, BSTR strXMLMenuToolbarIDCmdInfo, long lToobarRes)
{
	
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	CAddinInfo AddinInfo=CProjectFrameworkApp::m_AddinManager.GetAddinInfo(lSessionID);
	...
	{
		CProjectFrameworkApp::m_AddinManager.SetAddinInfo(lSessionID,AddinInfo); 
		return S_OK;
	}
	else
	{
...
/*-------------------------------------------- Function Header ---------------------------------------------*
	Function Name	  : CProjectFramework::LoadAllAdins
	Description	      : 
	Return Type	      : STDMETHODIMP 
	Warning           : 
  Argument          : BSTR strAddinLoadingInfo
------------------------------------------------------------------------------------------------------------*/

STDMETHODIMP CProjectFramework::LoadAllAdins(BSTR strAddinLoadingInfo)
{
...
STDMETHODIMP CProjectFramework::get_MainWindow(long *pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
...
STDMETHODIMP CProjectFramework::SetAddinToolbar(long lAddinIndex, VARIANT varValue)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
...
/*-------------------------------------------- Function Header ---------------------------------------------*
	Function Name	  : CProjectFramework::ParseAndLoadCommandInfo
	Description	      : Parses the XML document and loads all the informations into the data structure CAddinInfo
	Return Type	      : BOOL 
	Warning           : 
  Argument          : BSTR strXMLCommand
  Argument          : CAddinInfo &AddinInfo
------------------------------------------------------------------------------------------------------------*/

BOOL CProjectFramework::ParseAndLoadCommandInfo(BSTR strXMLCommand, CAddinInfo &AddinInfo)
{
	try
	{
		USES_CONVERSION;
...
BOOL CProjectFramework::GetAllCommandsInfo(CAddinInfo &AddinInfo, IXMLDOMNodeList *pXMLDOMNodeList)
{
...
STDMETHODIMP CProjectFramework::SetViewHeading(BSTR strHeading1, BSTR strHeading2, BSTR strHeading3)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	CProjectFrameworkApp::m_pView->m_strAddinProjectHeading1=strHeading1;
	CProjectFrameworkApp::m_pView->m_strAddinProjectHeading2=strHeading2;
	CProjectFrameworkApp::m_pView->m_strAddinProjectHeading3=strHeading3;
	CProjectFrameworkApp::m_pView->Invalidate(); 
	return S_OK;
}

STDMETHODIMP CProjectFramework::SendStatus(BSTR strStatus)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
